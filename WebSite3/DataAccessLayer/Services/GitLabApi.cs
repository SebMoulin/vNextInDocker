...
namespace DataAccessLayer.Services
{
    ...
    public class GitLabApi : IGitLabApi
    {
        ...
        {
            return await this.GetSecurityToken(_configProvider.GetAdminEmail(), _configProvider.GetAdminPassword());
        }
        ...
        public async Task<RestApiResponse<GitLabProject>> ForkRepository(string email, string password, DevEnv devEnv, string adminToken)
        {
            var sourceProject = await GetProjectTemplate(devEnv, adminToken);
            var userToken = await this.GetSecurityToken(email, password);
        ...
        public async Task<RestApiResponse<GitLabProject>> UpdateProjectVisibilityAndName(string projectId, string username, DevEnv devEnv, string adminToken)
        {
            var defaultName = _configProvider.GetDoNetTemplateRepoName();
            if (devEnv == DevEnv.Java)
            {
                defaultName = _configProvider.GetJavaTemplateRepoName();
            }
            var newName = string.Format("{0}-{1}", defaultName, username);
        ...
        {
            var users = await this.GetUsers(adminToken);
            return users.ToList().First(u => u.Username == username);
        }
        private async Task<GitLabProject> GetProjectTemplate(DevEnv devEnv, string adminToken)
        {
            var devEnvName = _configProvider.GetDoNetTemplateRepoName();
            var groupeName = _configProvider.GetCodingExerciseGroupName();
            if (devEnv == DevEnv.Java)
            {
                devEnvName = _configProvider.GetJavaTemplateRepoName();
            }
            var projects = await _httpRequestHandler.GetHttpRequest<GitLabProject[]>(string.Format("projects/search/{0}?order_by=created_at", devEnvName), adminToken);
